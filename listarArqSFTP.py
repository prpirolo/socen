...
try:
    ...
    nomeInicialArq = sPrefixo
    nomeInicialArqNOTFIS = sPrefixoNOTFIS

    # Alimenta a variável com os arquivos da pasta -> não processados
    print(nomeInicialArq+ ' - '+nomeInicialArqNOTFIS)
    ...
    for image in arqs:
        ...
        if nomeInicialArq in image:
            socen = socen + 1
        elif nomeInicialArqNOTFIS in image:
            notfis = notfis + 1
        else:
            outros = outros + 1

    print('Total de Arquivos Lidos: '+str(totarq)+' - SOCEN: '+str(socen)+ ' - NOTFIS: '+str(notfis)+ ' - OUTROS: '+str(outros))
